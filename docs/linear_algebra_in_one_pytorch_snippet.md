...
前面所有矩阵都是手写死的。这一节，我们让网络**自己学出一个变换**，并且把它**动起来看**。

> 想象把一张笑脸画在一张**橡皮纸**上。现在有人按某种固定规则把这张纸揉皱、拉扯，
> 纸上的脸就跟着变成了另一张脸。这个"怎么揉"的规则，就是一次**空间变换**——
> 第 2 节说的"矩阵就是把整个空间搬动的机器"，只不过这里的揉法是非线性的（真实人脸之间的变化也不是简单旋转拉伸）。
>
> **深度学习要干的事**：只给它"纸上每个点揉之前 / 揉之后的位置"，
> 让网络把这套揉纸规则**自己反推出来**。

核心就这么几行——网络学的是"平面上任意一点该搬到哪"：

```python
import numpy as np, torch, torch.nn as nn

# 真正的"揉纸规则"（网络看不到，只能从数据里猜）
def true_warp(p):
    x, y = p[:, 0], p[:, 1]
    return np.stack([x + 0.35*np.sin(1.5*y) + 0.10*y,
                     y + 0.30*np.sin(1.3*x) - 0.12*x], 1)

# 数据：纸上随机撒点，记录"揉之前 -> 揉之后"
X = np.random.uniform(-1.7, 1.7, (3000, 2)).astype("float32")
Y = true_warp(X).astype("float32")
...
> 画面里你会看到：一开始网络乱揉一通（随机初始化），脸是一团乱点；
