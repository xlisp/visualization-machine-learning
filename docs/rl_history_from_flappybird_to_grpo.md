...
## 第三幕:另一条路——策略梯度,直接学"该怎么做"

既然"先估价值、再挑动作"在动作太多时会崩,那能不能**跳过价值,直接学一个策略**?

这就是**策略梯度(Policy Gradient)**的思路。它不再问"这个动作值多少分",而是直接训练一个策略网络,输出"在当前状态下,各个动作应该以多大概率去做"。

它的学习逻辑朴素得惊人,可以概括成一句话:

> **如果一个动作带来了好结果,就提高它下次出现的概率;带来坏结果,就降低它的概率。**

最经典的策略梯度算法叫 **REINFORCE(1992)**。它的更新公式核心是:

```
loss = - log(动作的概率) × 这个动作最终拿到的回报
```

回报高的动作,被"按"着提高概率;回报低甚至为负的,被压低概率。多轮下来,策略就朝着"高回报"的方向进化了。
...
同一道题让模型答 8 遍、16 遍,有的对有的错——这"一组"就是 GRPO 里的 group。
...
基于价值(Value-based)                基于策略(Policy-based)
 ...
 擅长离散小动作空间                  +基线 → 优势 = 回报 - 平均
   │                                       │
 (动作太多就失效)                    PPO(2017) ── clip 稳定更新
        │                                   │
        │                            RLHF(ChatGPT) ── PPO+奖励模型,但太重
        │                                   │
        └──────────────────────────► GRPO(2024) ★MathGPT
                                       去掉奖励模型(规则打分)
                                       去掉价值网络(组内平均当基线)
...
从贝尔曼方程到 Q-Learning,从 DQN 到策略梯度,从 PPO/RLHF 到 GRPO——六十年里,方法在不断进化,但每一次进化的逻辑都一样:**前一个方法留下一个矛盾,后一个方法去解决它。**

- Q 表存不下 → DQN 用神经网络逼近;
- DQN 处理不了海量动作 → 策略梯度直接学策略;
- 策略梯度方差大、易学崩 → 优势基线 + PPO 裁剪;
- RLHF 太重 → GRPO 用规则打分和组内基线极致瘦身。
